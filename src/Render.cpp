...
void Render::drawNodeLines(short n) {
	...
	// [GEC]
	// Verifica si el jugador ha llegado completamente a su destino (posici�n y �ngulo coinciden con el objetivo).
	// Esto evita se marquen las lineas como visibles en el automapa cuando est�n fuera del nodo del BSP, 
...
void Render::addSprite(short n) {
	...
	// [GEC]
	// Verifica si el jugador ha llegado completamente a su destino (posici�n y �ngulo coinciden con el objetivo).
	// Esto evita se marquen cosas como visibles en el automapa cuando est�n fuera del nodo del BSP, 
	// ya que en algunas ocasiones se renderizan secciones del mapa sin que el jugador realmente se encuentre en ese nodo.
	//
	// Checks if the player has completely reached their destination (position and angle match the target).  
...
void Render::renderSpriteObject(int n) {
	...
	if (entity != nullptr) {
		...
		if (monster != nullptr) {
			// [GEC]
			// Verifica si el jugador ha llegado completamente a su destino (posici�n y �ngulo coinciden con el objetivo).
			// Esto evita que los enemigos se activen cuando est�n fuera del nodo del BSP, ya que en algunas ocasiones 
...
void Render::renderSpriteAnim(int n, int frame, int x, int y, int z, int tileNum, int flags, int renderMode, int scaleFactor, int renderFlags) {
	...
	bool isTorsoInFront = false; // Determina si el torso est� frente a la cabeza
	switch (monsterAnim) {
	case Enums::MANIM_IDLE_BACK: {
